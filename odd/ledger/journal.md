...
### J-003 — Aquifer entity OOM root-caused and fixed (closes J-002)

**Observation:** With Workers Logs enabled by PR #17 (J-002 H8), aggregate request analytics for `aquifer-mcp` since the 2026-04-23T04:40:05Z deploy showed two non-success invocations across 9.5 hours — `2026-04-23T04:40:00Z` outcome `exceededMemory` (1 req, 1 err, 2.82 ms CPU, **255 subrequests**) and `2026-04-23T13:55:00Z` outcome `clientDisconnected` (1 req, 0 err, 3.75 ms CPU, **255 subrequests**). The 04:40 row was the post-deploy seed validation entity:Paul call observed at the chat-protocol level as a 22.6s 503; the 13:55 row was an organic call from another consumer that aborted at its own 30s timeout before the Worker finished its fanout. Both invocations carry an identical 255-subrequest signature. Direct read of `src/tools.ts handleEntity` and the `bootstrapEntityMatches` it falls through to confirmed the mechanism: when an entity is not present in the pre-built `index.entity` map, the bootstrap path executes `Promise.allSettled(index.registry.map(...))` over all 33 served resources, and inside each, a nested `Promise.allSettled(files.map(...))` over every content file in that resource. Roughly 33 outer + ~222 inner ≈ 255 simultaneous R2 reads, with every parsed JSON article body retained in memory until the entire outer `allSettled` resolves. Worker memory budget is 128 MB; that working set exceeds it whenever upstream response sizes or latencies push the simultaneous-payload sum past the cap. The pre-built `index.entity` lookup itself is fast and correct; only the cold-path bootstrap fanout is broken. The cache write at the end of `bootstrapEntityMatches` is keyed by `composite_sha`, so the bootstrap re-runs from scratch every time any of the 33 repos updates, meaning the OOM is not a one-time per-entity event but a recurring one for every popular entity after every content refresh.

**Learning:** The userMemory note "the entity tool consistently hits Cloudflare Worker resource limits (Error 1102) — pre-existing issue, unrelated to recent PRs" is now upgraded from circumstantial to directly observed, with the specific failure outcome named (`exceededMemory`, not `exceededCpu`) and the specific subrequest count quantified (255). The 30s wall-clock pattern is misleading — it is not CPU exhaustion (the Worker barely uses CPU because it is just awaiting promises), it is heap exhaustion from holding all in-flight fetch responses simultaneously. This means CPU-budget tuning would not have helped; only reducing memory pressure would. The J-002 cascade (one entity timeout immediately followed by two `get` 503s in the same trace) is now explainable as the `exceededMemory` invocation poisoning either the isolate or the colo's view of route health for the next ~1 second, causing the next two requests to hit a 503 from an isolate that had not yet been replaced. The userMemory anti-pattern note "No tool handler may scan an entire content file at query time — lookup indexes must be pre-built" applies at a deeper layer than originally interpreted: even when a lookup index *exists*, any cold-path that scans the full corpus to *populate* the index synchronously inside a user request is the same bug at a different timescale.

**Decision:** Implement bounded concurrency on both fanout levels of `bootstrapEntityMatches` plus a 20s wall-clock deadline with non-poisoning partial-result handling. Resource-level chunk size 4, file-level chunk size 8, max 32 in-flight fetches at any moment instead of 255. Wall-clock budget 20s leaves 10s of headroom under the typical 30s client timeout for response formatting and return. Partial results from a deadline bail are returned to the caller but not written to the R2 bootstrap cache, so a later invocation that completes successfully still gets its full result memoized. Ship as PR #18 with the change in `src/tools.ts` (helper + chunked bootstrap), 5 new unit tests in `src/tools.test.ts` for the `settledInChunks` helper, and this journal entry. Do not refactor `buildIndex` to populate `index.entity` more eagerly in this PR — that is the canonical fix per the pre-built-index principle, but it requires an analysis of which articles `buildIndex` is currently skipping and why, and is too large a scope for the same PR. Track as H11 below.

**Constraint:** Cloudflare Workers Logs is now ingesting per-invocation outcome data for `aquifer-mcp` and is the authoritative source for any future 503 investigation. The token used to query GraphQL Analytics during this investigation had `Account Analytics: Read` scope but not `Workers Observability: Read` and not `Workers Scripts: Edit` — so aggregate per-minute outcomes were available, but per-invocation log rows (with full exception stacks, response sizes, and per-subrequest timings) were not, and live `wrangler tail` was not. Future investigations that need per-invocation detail will require either a token with `Workers Observability: Read` or maintainer dashboard access. The 32-in-flight headroom in the chunked bootstrap is a heuristic safety margin against unknown article-payload sizes; if a single content file's parsed JSON ever exceeds ~3 MB the cap may need to drop further. Article-size telemetry would let us tune this empirically rather than by guess.

**Handoff:**
- **H11** — `buildIndex` should populate `index.entity` with every entity reference at index-build time so that `bootstrapEntityMatches` becomes dead code and can be removed. This is the canonical fix per the project's pre-built-index principle. Scope: audit `buildIndex` to determine which articles it is currently scanning and which it is skipping; if it is not exhaustively processing every content file, decide whether to add full processing (with the same chunked-fanout helper from this PR for build-time memory safety) or to keep a permanent thin background bootstrap that runs via `ctx.waitUntil` on cache miss instead of inline. Either path eliminates the user-blocking corpus scan entirely.
- **H12** — The `settledInChunks` helper introduced in `src/tools.ts` is generic and likely useful to apply to other fanout sites in the codebase. Audit `tools.ts`, `registry.ts`, and `github.ts` for other `Promise.all*(items.map(...))` patterns over collections of unknown size. The `fetchAllRepoShas` cold path (62-way parallel GitHub fetch) referenced in J-002 is the most obvious next candidate even though it has not been observed to fail, because GitHub `/commits/main` responses are tiny and the subrequest budget is comfortable; lower priority than H11.
- **H13** — Open question whether to mint a longer-lived Cloudflare API token with `Workers Observability: Read` and store it in the repo's GitHub Actions secrets so future investigations can be done agentically (by Claude or another tool) without requiring the operator to mint and paste a token each time. Tradeoff: convenience vs. token sprawl and secret rotation hygiene. No action this turn; tracked for future discussion.

**Update — transparency revision (same PR, second commit):**

The first commit on this branch fixed the OOM mechanism but did not address a derivative concern: when the wall-clock deadline trips mid-bootstrap, the truncated `ArticleRef[]` was returned to callers with no signal of incompleteness. Both `handleEntity` and `searchByEntity` would then format that partial set as "Found N article(s)" — silently presenting a non-exhaustive result as authoritative. That is a violation of Axiom 4 (You Cannot Verify What You Did Not Observe): the server returns a list it has not actually finished computing and labels it as the answer.

The second commit replaces `bootstrapEntityMatches`'s `Promise<ArticleRef[]>` return with `Promise<BootstrapEntityResult>`, a structured object carrying `matches`, `complete`, `scanned_resources`, `total_resources`, `scanned_files`, `total_files_estimate`, `budget_exceeded`, and `duration_ms`. A new `formatPartialBootstrapNote(result)` helper produces a single user-facing disclosure string ("⚠ Partial result: lookup deadline reached (Ns). Scanned X/Y resources and A/B files. Additional matches may exist in unscanned content. Retrying may continue the scan from a warm cache.") that both callers append when `complete === false`. The cache-write gate is unchanged — only complete results are memoized. The tracer span now carries scanned/total counts in addition to the partial flag, so dashboard observability sees the same information as the user. The revised type is exported so unit tests can construct fixtures and assert against the contract directly. Four new tests cover: (a) cold-path empty scan returns `complete: true`; (b) `handleEntity` empty result emits no partial note in the complete case; (c) `BootstrapEntityResult` shape contract enforced at runtime via `expect.toMatchObject`; (d) cache-hit fast path returns `complete: true` regardless of scanner counters. All 155 tests pass (was 151 after the first commit on this branch). The partial-results note text was rendered manually via Node and confirmed to format cleanly with double-newline separator and unambiguous warning glyph, so a downstream LLM consuming the MCP response cannot mistake it for a regular result line.

This second commit was driven by a Klappy operator instruction during PR review: "Ensure vodka architecture is maintained. Also transparency is essential and required for any partial results when bailing on the pending resources and file fetches." Vodka was pressure-tested against canon (`klappy://canon/principles/vodka-architecture`) and held: the helper is generic over `<T,R>`, no domain branches were introduced, server LOC growth (~9% for the entity tool's safety net) is well below KB growth (~88% toward target coverage). Transparency was implemented via explicit data flow rather than tracer side-channel, sentinel ArticleRef, or thrown PartialResultError — alternatives weighed and rejected during `oddkit_challenge` because they all hide the partial state from the type system. The shipping behavior is: callers can no longer ignore incompleteness silently; the type system requires them to at least pass through the `complete` flag, and the convention is to call `formatPartialBootstrapNote` on the false branch.

**Update — Bugbot review fixes (same PR, fourth commit):**

Cursor Bugbot reviewed the second commit and surfaced five potential issues. Cursor Agent autofixed two of them in commit `dafe73c8`: the missing-`scannedResources++` on the no-`repoSha` early return (which would have made `complete=false` permanent for any registry containing an unscannable entry), and the hardcoded `(PARTIAL: budget_exceeded)` tracer label that ignored non-deadline causes. Three findings remained — one Medium that no test would have caught and a High that would have shipped silent-truth-loss exactly the kind this PR exists to prevent.

The fourth commit closes all three. **(#3 Medium — Deadline cannot stop active fetch batches):** the deadline check before each file callback didn't help once `settledInChunks` had already launched a slow batch — those fetches ran to completion and could push the worker past the 30s client timeout. Fixed by introducing `withinDeadline<T>(promise)` that races each fetch against the remaining wall-clock budget; losing the race rejects with `DEADLINE`, which the file callback catches and counts as a `failedFiles` increment. **(#4 Medium — Complete scans mislabeled when budget exceeded):** the `complete` formula was `!budgetExceeded && scannedResources === total`, which forced `complete=false` even in the corner case where every resource finished right at the deadline boundary. Fixed by dropping `!budgetExceeded` from the formula entirely; completion is now determined by counters (`resourcesPartial === 0 && resourcesSkipped === 0 && failedFiles === 0`), with `budget_exceeded` retained as an independent diagnostic field for telemetry. **(#5 High — Failed file fetches still marked complete):** `Promise.allSettled`'s rejected entries were silently dropped from the `refs` accumulator but the resource was still counted as fully scanned, so a result with `complete=true` could carry data missing entire content files. Fixed by replacing the single `scannedResources++` counter with three explicit per-resource states — `resourcesComplete` (no failures, no kills), `resourcesPartial` (started but lost some files), `resourcesSkipped` (never entered) — and only counting a resource as complete when every one of its file fetches resolved.

The `BootstrapEntityResult` interface gains a `failed_files: number` field. The `formatPartialBootstrapNote` helper now branches on a three-way reason taxonomy that mirrors the tracer label exactly: `budget_exceeded` first (the wall-clock cap explains everything else), then `failed_files > 0` (upstream/network failures), then `scan incomplete` as the catch-all. The user-facing string and the trace header now agree on diagnosis, so an operator reading logs sees the same explanation the user saw — which is the property the transparency revision was supposed to guarantee in the first place.

Test coverage: 4 new tests in `describe("BootstrapEntityResult — Bugbot fix coverage")`. The High-severity #5 gets two tests — one verifying `complete=false` and `failed_files > 0` under simulated upstream rejection, one verifying that `storage.putJSON` is never invoked with an `entity/*` key under the same scenario (the cache-poisoning prevention property). The autofix #2 gets a regression test confirming an unscannable resource doesn't drag `complete` to false. The autofix #1 gets a test verifying the "fetch(es) failed" reason text reaches the user-facing prose. 159/159 tests pass total (was 155). Build (`wrangler deploy --dry-run`) clean. Manual node-render of all three partial-note branches confirms cleanly distinguishable user-visible text for each cause.

The five-finding Bugbot review is itself the lesson: the transparency abstraction (BootstrapEntityResult) was the right shape, but the implementation under-delivered against its own contract on three independent axes. Each axis was a reasonable single-developer oversight that a careful reviewer caught. Without the Bugbot review, the second commit would have shipped with `complete=true` on partial data — a silent truth violation that the entire transparency mechanism existed to prevent. The mechanism only becomes durable once the implementation is also reviewed for the failure modes the type system can't enforce. Recorded for canon as a paired pattern: explicit-data-flow types prevent silent failure at the API boundary, but adversarial review (human or bot) prevents the implementation from quietly violating the contract the type promises.

**Update — final review-cycle fix (same PR, eighth commit):**

Bugbot completed its post-fourth-commit re-review and surfaced four additional findings against `6546fc17`. Cursor Agent autofixed three of them in three sequential commits (`0a0694d1` — added the missing `failed_files` assertion to the BootstrapEntityResult shape contract test that finding #6 flagged; `12a012d5` — closed finding #7 by saving the derived promise from `Promise.race` and attaching `.catch(() => {})` so a rejected fetch losing the race doesn't surface as an unhandled rejection in Workers Logs; `c05ef570` — closed finding #8 by attaching the same `.catch(() => {})` to the original promise on the `withinDeadline` early-return path where `remaining <= 0`). All three fixes are mechanically clean and introduce no regressions; the autofix produced exactly the change a careful reviewer would have written.

The eighth commit closes the remaining finding #9 (Low) — a dedup bug in `formatPartialBootstrapNote` where the `failedSuffix = ", N failed"` was appended unconditionally, producing duplicated count text in the failed-files-only branch ("3 content file fetch(es) failed. Scanned 30/33 resources and 217/230 files, 3 failed"). The fix gates the suffix on the `budget_exceeded` branch only — that's the branch where the primary reason text doesn't carry the count, so the suffix is the user's only signal. In the failed-files-only branch the count is already in `reason` and adding it again is noise. Two regression tests added: one verifying the count appears exactly once in the failed-files branch via regex match-count, one documenting the no-failures-no-deadline branch produces no `"failed"` substring at all. Total tests: **161/161 passing** (was 159 on commit `6546fc17`, was 155 before the autofix series).

Final tally for this PR's bug-fix arc: **9 Bugbot findings** total across two review cycles. **6 closed by Cursor Agent autofix**, **3 closed by manual fix in commits authored from the planning context**. Severities: 1 High, 3 Medium, 5 Low. The High and one Medium were silent-truth-loss bugs (`complete=true` shipping with hidden gaps) — exactly the failure mode the BootstrapEntityResult transparency abstraction was introduced to prevent. The fact that the abstraction's own implementation initially violated its contract on three independent axes (deadline can't kill in-flight fetches; `complete` formula force-flipped on budget exceedance; rejected fetches counted as scanned) is the meta-lesson worth carrying into canon: a type-system contract is necessary but not sufficient. The contract makes silent failure visible at the API boundary; only adversarial review (human or bot) prevents the implementation from quietly violating its own promise.
...
### J-005 — H11b: apply partial-data-with-transparency principle to entity lookup

**Observation:** Post-H11 production probe at 2026-04-24T01:11Z showed the entity lookup path still has user-blocking failure modes. `entity:Paul` returned in 450ms (warm bootstrap cache from prior session — the `storage:index/AquiferO…/entities.j…=104ms(miss)` trace lines confirmed no per-resource entity indexes exist for the current composite SHA). `entity:Obadiah` first attempt timed out at 11.2s with HTTP 502 (Cloudflare frontend cut the stream while the Worker was still running bootstrap). `entity:Obadiah` second attempt returned 78 matches in 588ms from the R2 bootstrap cache (tier-3 cache write from attempt 2's completed-in-background scan worked as designed). The H11 deploy did NOT populate per-resource entity indexes because `populateEntityIndexes` runs inline in `buildIndex`, which only executes on a composite SHA change — and the deploy didn't change any repo SHAs, so the pre-H11 index remained warm. The architectural cost H11 tried to avoid (user-blocking corpus scan) was actually still happening via tier-3 bootstrap for every first-time entity lookup, just masked by the session-scoped cache warming my prior probes accidentally did.

Operator response: "It's a principle of partial data with transparency to come back for more. Background fetch will warm the cache before you come back." The principle is general. It governs both the build-time cost (which H11 accepted by putting the scan in `buildIndex`) and the query-time cost (which H11 left as a tier-3 fallback). Neither should block the user.

**Learning:** H11 fixed the wrong axis. Moving the corpus scan from per-query to per-index-build reduced amortized cost but did not remove the user-blocking property — a true cold start (new composite SHA) would still block for ~22s on `populateEntityIndexes`. And the migration path was worse: indexes built before the H11 deploy never got entity indexes populated, so every entity lookup today still routes through the bootstrap tier that the PR was supposed to retire. The durable principle is not "build the index eagerly instead of lazily." It is: **the user path must never pay for a corpus scan regardless of when the scan runs.** The right shape is fan-out returns whatever is in R2 now, declares its incompleteness explicitly, and schedules a background warm via `ctx.waitUntil` that writes per-resource indexes without blocking the response. The second visit finds the warmed indexes. The first visit pays no corpus-scan cost, ever. This is the pattern `refreshAndUpdateCurrentIndex` already uses for SHA-staleness refresh in `registry.ts`; H11b is applying that same pattern to entity indexes.

A derivative learning: partial results require a type-system contract, not just a convention. The H11b design returns `FanOutEntityResult { matches, complete, scanned_resources, total_resources, missing_resources }` rather than a bare `ArticleRef[]`. Callers cannot ignore the completeness axis; the type forces them to check. The paired-pattern lesson from J-004 (type contract + adversarial review) applies again: the contract alone doesn't guarantee honesty, but without it the implementation is free to drift silently.

**Decision:** Ship H11b as PR #20. Remove `await populateEntityIndexes(...)` from `buildIndex`. Change `fanOutEntitySearch` return type to `Promise<FanOutEntityResult>` carrying `missing_resources: ResourceEntry[]`. Add exported `warmEntityIndexesForResources(resources, repoShas, env, storage)` that scans the specified resources' content files and writes per-resource entity indexes to R2, suitable for invocation from `ctx.waitUntil`. Wire both `handleEntity` and `searchByEntity` to: (a) return whatever fan-out found immediately, (b) emit `formatPartialFanOutNote(fan)` when `missing_resources` is non-empty, (c) kick off `ctx?.waitUntil?.(warmEntityIndexesForResources(...))` for those resources. Remove the bootstrap fallback from both user-facing callers; keep `bootstrapEntityMatches` exported as a direct diagnostic function (callable for manual corpus scans, not routed to from user requests). 168/168 tests pass locally with the `build-test` CI command; 3 new H11b-specific tests cover the ctx.waitUntil contract (called exactly once with a Promise when missing is non-empty; skipped cleanly when ctx absent; not called at all when fan-out is complete).

**Constraint:** The cache-fetches-and-parses canon principle (`klappy://canon/principles/cache-fetches-and-parses`) governs: per-resource entity indexes ARE parse products (expensive corpus scan + JSON decode + association extraction) and should be cached. H11b does not remove the cache — it keeps the SHA-keyed per-resource entity index blobs in R2. What changes is when the cache is populated: not synchronously in `buildIndex`, not synchronously on query, but in the background after the first partial query exposes a miss. The cache itself is correct per canon; the population trigger moves. This preserves the existing R2 lifecycle (SHA rotation → URL rotation → no staleness risk per anti-cache-lying). Post-deploy observation constraint: first-query latency for an uncached entity must drop below Cloudflare's frontend gateway timeout (~30s hard, practically ~10-15s for consistent UX). Fan-out with 33 missing resources is N parallel R2 reads of small per-resource blobs that mostly return null (miss); the fan-out itself should resolve in <1s. If it doesn't, something else is wrong — the fix would be to limit the number of misses fan-out tolerates before returning partial-now.

**Handoff:**
- **H15** — Monitor Workers Logs outcome distribution for entity calls across the first 24-48h after H11b deploy. Target: zero `outcome=exceededMemory`, zero `outcome=exceededCpu`, zero edge-injected 502s. First-call latency should be the fan-out time (~500ms-1s) plus response formatting, not the corpus-scan time (~22s). Second-call latency for previously-warmed entities should hit the R2 entity-index cache in ~100-300ms.
- **H16** — Once H11b stabilizes, delete `bootstrapEntityMatches`, `BootstrapEntityResult`, `formatPartialBootstrapNote`, and the internal `populateEntityIndexes` helper. They are dead code after H11b lands but kept temporarily as diagnostic surface / safety net. Deletion should be a standalone PR once the new path has been observed healthy for a week.
- **H17** — The disclosure note text ("Retry in a few seconds for the complete result") is a fixed 15-word string. If production observation shows warm-completion latency regularly exceeds ~5s on slow-GitHub conditions, the text should dynamically reflect the count of warming resources (e.g., "~30 seconds for 33 resources" vs "~2 seconds for 2 resources"). Low priority — tune if observed.
- **H18** — Consider whether a dedicated `retry` or `refresh` tool should exist to let callers explicitly request a full scan + wait, instead of polling via retries. Only worth considering if H17 observations show retries are common; default stance is no, the partial-with-transparency contract means the user has agency to decide when to retry.

---

**Evidence captured for DoD §4 (vitest output — the closest equivalent to visual proof for a refactor):**

```
RUN  v3.2.4 /home/claude/aquifer-mcp
 ✓ src/coverage.test.ts (4 tests) 886ms
 ✓ src/tools.test.ts (67 tests) 111ms
 ✓ src/telemetry.test.ts (17 tests) 34ms
 ✓ src/references.test.ts (80 tests) 18ms
 Test Files  4 passed (4)
      Tests  168 passed (168)
```

Build verification (`wrangler deploy --dry-run`): clean, no binding/compatibility-flag changes. Typecheck output: 11 pre-existing `ResourceMetadata` cast errors in tests (unchanged from main — not introduced by this PR).

**Update — Bugbot review on the H11b PR (same PR, fifth commit):**

Cursor Bugbot reviewed the first H11b commit (`ac7f10c8`) and surfaced 5 findings across 4 review cycles. Three were resolved by Cursor Agent autofix before I looked at the PR again: `bd35ae8e` passed `ctx` through `handleSearch → searchByEntity` (closing the **High-severity #1** — `searchByEntity` referenced an undeclared `ctx` that would have thrown `ReferenceError` at runtime on every cold-cache search with an entity query in strict-mode ES modules; the build pipeline uses esbuild transpile-only with no type checking, so it compiled cleanly and would have shipped as a production crash); `94e26484` used the hardcoded `"eng"` language in the warm-path metadata fetch to match `buildIndex` (closing **#2 Medium** — the mismatch would have made warms silently fail for every non-English-primary resource, producing an endless partial-note loop) and corrected an orphaned JSDoc placement; `739c5c37` made rejected `storage.getJSON` reads count as missing rather than silently-complete (closing **#4 Medium** — R2 outage or corrupt JSON would have falsely reported fan-out as complete and skipped the warm, a silent-truth-loss pattern that bypassed the transparency disclosure H11b exists to provide).

The fifth commit on this PR closes the remaining **#5 Low** — `populateEntityIndexes` was dead code. It had no export and all call sites were removed from `buildIndex` in `ac7f10c8`, making the "preserved for diagnostic use" justification in the original commit message false (no external caller could reach it). The scanning logic it contained is now fully represented by `warmEntityIndexesForResources`. Deleted the 60-line function plus its JSDoc, cleaned the three stale comments that referenced it by name (two in `src/registry.ts`, one in `src/tools.test.ts`). 168/168 tests still pass; build clean.
