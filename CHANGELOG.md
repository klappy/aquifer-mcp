...
- **Complete X-Ray trace coverage**: Tracer now threaded through every I/O function in the tool handler chain — `fetchContentFile`, `getResourceMetadata`, `resolveIndexReference`, `listContentFiles`, `buildCatalog`, `buildCatalogFast`, and `bootstrapEntityMatches` all pass the tracer to `fetchJson`/`storage.getJSON`. Aggregate spans added for `scripture-fetch` (Bible parallel fetch with hit count), `find-article` (lookup-hit vs fallback path), and `entity-bootstrap` (resource count + match count). `fetchJson` in `github.ts` now accepts and passes tracer to `storage.getJSON`. The ~1,200ms untraced gap between index load and response is now fully visible in X-Aquifer-Trace.

### Added

- **`src/version.ts`**: Auto-generated single source of truth for the VERSION constant. `prebuild` script in `package.json` generates it from `package.json` version field.
- **`prebuild` script**: Runs before `build`, `deploy`, and `deploy:*` scripts to keep `src/version.ts` in sync with `package.json`.
